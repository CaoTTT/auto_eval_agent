...
运行中仍可继续提交新的全量评测任务。后端按 FIFO 队列逐个启动任务，任意时刻只运行一个全量批跑；每个任务开始后，仍按它提交时设置的 `concurrency` 并发评测内部题目。页面顶部的“任务队列”会显示当前运行项、等待顺序和进度，尚未开始的任务可点击“取消排队”。`GET /api/queue` 可获取队列状态，`DELETE /api/queue/{task_id}` 可取消等待任务。队列驻留在当前服务进程中，服务关闭时尚未开始的排队任务会标记为中断。
