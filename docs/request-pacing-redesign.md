...
页面的 Case 并发容量范围仍为 1–128。它控制任务内同时处理的题数，不是每秒调用次数。独立 Case 持续补入，同一多轮会话按轮次顺序执行，任务之间仍为 FIFO。视频抽帧、长截图准备和图像编码共用 4 个预处理槽位。
...
平滑发送间隔额外保留 1 ms 余量，并受 Token 消耗、冷却和滚动窗口共同约束。空闲不会累积可集中消费的发送许可，事件循环暂停后不补发错过的额度。启动及超过 30 秒的空闲后，从目标速率的 25% 开始，15 秒逐步恢复。

## 真正发送前检查

业务层取得许可后，HTTP 连接池仍可能排队。因此本次将实际生产入口移到 HTTPcore 的 `http11.send_request_headers.started` 事件：连接池、TCP/TLS 已就绪，接下来写入请求头。

一个共享锁从请求头开始写入持有至完成/失败，仅串行发送请求头，不等待响应正文。完成写入时保守更新发送时间和下次允许发送时刻，避免 socket 写入过程中的暂停导致旧许可集中放行。模型的流式响应保持异步并行。
...
请求时间戳和 Token 账本独立存储，输出结算不会增加 RPM/RPS 计数。未完成响应的 Token 预留不会在一分钟后自动消失；收到 usage 后结算实际输入和输出。无 usage 的失败或取消使用保守预留，避免本地释放后立即超发。

输入/输出估算根据返回 usage 校正。请求数限制可在本地准确计数，Token 预算是估算，不能视为服务端真实 TPS 计量。大图和长输出仍可能触发 Token 限流。
...
本次验证：全量回归369项通过；随后补充官方错误码分类、恢复、重定向及客户端入口测试，最终相关专项70项通过。最终共收集378项Python测试，均由上述全量与专项覆盖；6个Node前端测试脚本通过。真实Vue页面使用合成状态完成截图检查，无脚本错误。复查发现的重定向重复trace自锁问题已修复，并用真实307跳转、连接池/在途容量均为1的测试验证。
