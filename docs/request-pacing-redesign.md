...
> 本文保留初版实现和历史验证数据。2026-09-17 已提高两种模型的默认预算，并支持通过评估并发数控制流量；当前配置、旧任务恢复和部署要求以[裁判模型与思考模式](judge-model-thinking.md#限流与部署范围)为准。
...
平滑发送间隔额外保留 1 ms 余量，并受 Token 消耗、冷却和滚动窗口共同约束。空闲不会累积可集中消费的发送许可，事件循环暂停后不补发错过的额度。启动及超过 30 秒的真正空闲后，请求频率从目标速率的 25% 开始，15 秒逐步恢复；此预热只作用于请求频率，不再放大 Token 发送间隔。等待发送额度、等待连接或仍有模型响应在途时不算空闲。

2026-09-16 修正：下一次发送间隔取 `max(60 / 目标RPM / 预热系数, 本次预估Token × 60 / 目标TPM) + 0.001秒`。默认 800,000 TPM 下，本次总预估为 50,000 / 100,000 / 150,000 Token 时，启动后的下一次发送间隔分别约为 3.75 / 7.5 / 11.25 秒，原来分别为 15 / 30 / 45 秒。空控制器的首次请求仍立即准入；Token 余额不足、请求窗口已满、在途已满或限流冷却时仍等待。这里比较的是本地调度间隔，不是单题总耗时或真实模型加速倍数。
...
大请求回归使用每次 500,000 Token 的估算值。早期修正避免了预热被反复重置而一直等待约 150 秒；2026-09-16 的修正进一步取消首次和空闲恢复后的 Token 间隔放大。在其他限制允许时，从首个请求之后即按预算对应的约 37.5 秒发送间隔执行。该数字仅说明限速计算结果，不是模型耗时实测。状态接口的 `pending` 包含等待发送锁的全部请求，`target_rps` 是目标上限，并不表示实际每秒已发送的数量。

2026-09-16 定向回归：103 项通过，覆盖调度核心、HTTP 传输、流式重试、阶段计时、媒体准备流水线和状态接口。新增用例验证空控制器首发无延迟、50,000 / 100,000 / 150,000 Token 的启动间隔、空闲恢复后的大请求间隔，以及 Token 用满后必须等待 usage 结算释放余额。验证使用虚拟时钟、合成数据和本地 HTTP 服务，没有调用真实百炼模型。
