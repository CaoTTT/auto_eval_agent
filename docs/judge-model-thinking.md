...
页面「②评估配置 → 评估并发数」可设置 1–128，两种模型默认均为 128。并发数控制同时处理的题数，调低即可减少同时准备或等待响应的题目，实际发送量同时受请求/Token 预算约束；并发数不等于每秒请求数。已运行的任务先暂停，等待已开始题目保存完成，再修改「恢复并发数」并继续。切换模型保留手动并发值，但不超过新模型配置的推荐上限。
