...
- `exports.py`：独立后台导出，不进入评测 FIFO 队列。最多接受 4 个未完成导出，同一任务正在生成时复用导出 ID；每次只执行一个后台生成任务。2026-09-22 更新：新旧 HTTP 入口统一通过异步队列等待，避免旧入口在工作线程中等待生成锁；工作簿生成器仍保留进程内信号量保护直接 Python 调用。
- `history.py`：新增 `write_xlsx(snapshot, destination)`，直接写临时文件。保留 `build_xlsx()` 字节返回接口供既有调用方使用，复用同一份 OOXML 生成逻辑。
- `server.py`：保留 `GET /api/eval/{task_id}/export?format=xlsx`，返回文件响应。2026-09-22 更新：该入口与后台接口共用导出任务、容量限制和文件保留策略；默认不包含原图，需要时传入 `include_images=true`。新页面使用下表的后台接口。
...
2026-09-22 更新：下载中的文件会延后到所有下载结束后再清理，传输失败或取消也会释放文件保护。多人导出的处理规则和验证范围见[对比导出说明](task-comparison-export.md#多人同时导出2026-09-22)。
